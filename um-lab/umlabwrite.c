...
void test_copy_segment(int segment_id){
     set_register(1, 0);
     set_register(2, 1);
     int rb = get_register(1);
     int rc = get_register(2);
     Sequence s1 = memory[0];
    copy_segment(rc);
    if(*segment_counter != 0 && program_counter !=rc){
          fprintf(stderr, "Wrong counter value \n“);
          return;
    }
    Sequence s2 = memory[rb];
    int l = Seq_length(s2);
    if(s2 != s1){
        fprintf(stderr, "Wrong copied sequence length \n“);
        return;
    }
    for(int i = 0; i < l;  i++){
        if( s1[i] != s2[i]){
            fprintf(stderr, “Old value not matching copied value \n“);
        }
    	return;        
    }
}
